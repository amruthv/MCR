...
def experiment1(shouldDraw = True, shouldSmooth = True):
    ...
    rrt = RRT(start, 0.1, goal, robot.generateRandomConfiguration, collisionChecker.validateTrajectory)
    rrt.growUntilFound()
    cameFrom = rrt.cameFrom

    path = searcher.reconstructPath(cameFrom, tuple(goal))
    if path == None:
        print 'path was None'
        return None
    if shouldSmooth:
